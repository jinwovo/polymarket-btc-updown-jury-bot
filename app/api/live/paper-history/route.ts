...
const globalCache = globalThis as typeof globalThis & {
  __fp_paper_history_cache?: PaperHistoryCache;
};

function cacheState(): PaperHistoryCache {
  if (!globalCache.__fp_paper_history_cache) {
    globalCache.__fp_paper_history_cache = {
      ts: 0,
      payloadByKey: new Map(),
      inFlightByKey: new Map(),
    };
  }
  return globalCache.__fp_paper_history_cache;
}
  ...
  try {
    const req = (async () => {
      const response = await fetch(
        `${PY_API_BASE}/api/paper-history?limit=${encodeURIComponent(limit)}&offset=${encodeURIComponent(offset)}`,
        { cache: "no-store" },
      );
